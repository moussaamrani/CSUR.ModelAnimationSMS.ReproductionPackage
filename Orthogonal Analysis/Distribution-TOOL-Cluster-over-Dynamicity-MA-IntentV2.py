...
fig, (a1, a2) = plt.subplots(ncols=1, nrows=2, sharex=True, figsize=(18,7), constrained_layout=False)
fig.subplots_adjust(top=0.95, bottom=0.12, left=0.08, right=0.96, hspace=0.3)  # adjust margins to put labels outside
...
# Labels and styling (put outside figure)
...
a1.set_xlabel('')
a2.set_xlabel('TOOLS', size=18, labelpad=30, weight="bold")
a1.set_axisbelow(True)
a2.set_axisbelow(True)
a1.grid(ls='dotted', color='black')
a2.grid(ls='dotted', color='black')

# Reduce internal whitespace inside the plots
a1.margins(0.05)
a2.margins(0.05)

# Set exact axis limits to align points with borders
a1.set_xlim(-0.5, len(category)-0.5)
a1.set_ylim(-0.5, len(tech)-0.5)
a2.set_xlim(-0.5, len(category)-0.5)
a2.set_ylim(-0.5, len(level)-0.5)

# Show and save figure
plt.show()
fig.savefig('TOOL+DYN+MAIntent.pdf', dpi=200, bbox_inches='tight')
